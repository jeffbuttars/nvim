-- AIDEV-NOTE: VENDORED copy of Neovim 0.12.2 runtime pack nvim.difftool/lua/difftool.lua.
-- Loaded by the difftool.lua dispatcher (default engine). Sole local change vs upstream: nil-guard
-- on `rel` in diff_dirs_diffr (see AIDEV-NOTE below) — upstream stores nilable vim.fs.relpath()
-- unguarded, crashing the sort in diff_dirs. Re-sync or DELETE this file when nvim upgrades / once
-- the upstream fix lands. Diff against the bundled copy to spot drift. Toggle: see difftool.lua.
--- @brief
---<pre>help
  ...
  for _, line in ipairs(lines) do
    ...
    if modified_left and modified_right then
      ...
      table.insert(qf_entries, {
        ...
        user_data = {
          diff = true,
          -- AIDEV-NOTE: relpath is nilable (returns nil when target not under base); a nil rel
          -- crashes the table.sort in diff_dirs. Fall back to the full path to stay sortable.
          rel = vim.fs.relpath(left_dir, modified_left) or modified_left,
          left = left,
          right = right,
        },
      })
